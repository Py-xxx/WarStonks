...
function OpportunityCard({
  ...
  return (
    <article className={`opp-card opp-card-${opp.category}${pinned ? ' is-pinned' : ''}`}>
      ...
      <div className="opp-card-foot">
        <span className={`opp-conf ${confidenceClass}`} title={`Confidence: ${opp.confidenceLabel}`}>
...
export function OpportunityBoard() {
  ...
  // Pinned ("accepted") quests sit on top and never disappear; the rest of the board is whatever
  // the recompute produced, minus anything already shown as pinned.
  const pinnedAll = useMemo(
    () => Object.values(pinnedOpportunities).sort((a, b) => b.score - a.score),
    [pinnedOpportunities],
  );
  const unpinnedAll = useMemo(
    () => opportunities.filter((opp) => !(opp.subjectKey in pinnedOpportunities)),
  ...
  const pinnedList = pinnedAll.filter(activeMatch);
  const unpinnedList = unpinnedAll.filter(activeMatch);
      ...
      {unpinnedList.length === 0 && pinnedList.length === 0 ? (
        <div className="opp-board-empty">
          {loading && loadedAt === null
            ...
            : activeFilter !== 'all' && (unpinnedAll.length > 0 || pinnedAll.length > 0)
              ? `No ${
                  BOARD_FILTERS.find((filter) => filter.id === activeFilter)?.label ?? ''
