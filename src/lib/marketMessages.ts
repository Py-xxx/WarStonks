...
export function formatWhisperMessage(target: WhisperTarget, itemName: string): string {
  ...
  return `/w "${target.username}" Hey there! I would like to buy ${formatWhisperItemName(itemName)}${rankSuffix} for ${target.platinum} :platinum: please (WarStonks via warframe.market)`;
}
