...
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PaddleOcrCell {
    pub row_id: String,
    pub tile_index: usize,
    pub name_box: Option<PaddleOcrBox>,
    pub quantity_box: Option<PaddleOcrBox>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct PaddleOcrReading {
    pub row_id: String,
    pub tile_index: usize,
...
fn resolve_paddle_ocr_script_path(app: &tauri::AppHandle) -> Result<PathBuf, String> {
...
fn extract_data_url_payload(image_data_url: &str) -> Result<String, String> {
    image_data_url
        ...
        .ok_or_else(|| "Invalid screenshot image payload.".to_string())
}

#[tauri::command]
pub async fn run_set_completion_paddle_ocr(
    app: tauri::AppHandle,
    image_data_url: String,
    cells: Vec<PaddleOcrCell>,
) -> Result<Vec<PaddleOcrReading>, String> {
    tauri::async_runtime::spawn_blocking(move || {
        let script_path = resolve_paddle_ocr_script_path(&app)?;
        ...
        if let Some(mut stdin) = child.stdin.take() {
            stdin
                ...
                .map_err(|error| format!("Failed to send OCR payload: {error}"))?;
        }
        ...
        serde_json::from_slice::<Vec<PaddleOcrReading>>(&output.stdout)
            .map_err(|error| format!("Failed to parse PaddleOCR output: {error}"))
    })
    .await
    .map_err(|error| error.to_string())?
}
